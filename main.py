...
class CopilotLocalServer:
    ...
    async def _prepare_chat_headers(self):
        try:
            token = await self.plugin.get_session_token()
        except Exception as e:
    ...
    async def _proxy_chat_request(self, headers, payload, is_stream, request):
        if is_stream:
    ...
    async def handle_chat(self, request):
        ...
        headers, err_resp = await self._prepare_chat_headers()
        if err_resp:
            return err_resp
        ...
        if "tools" in req_data: payload["tools"] = req_data["tools"]
        if "tool_choice" in req_data: payload["tool_choice"] = req_data["tool_choice"]
...
class GithubCopilotPlugin(Star):
    ...
    async def copilot_usage(self, event: AstrMessageEvent):
        ...
        try:
            user_info, limits_info = await self._fetch_user_quota(ghu_token)
        ...
        msg = (
            "👩‍💻 **GitHub Copilot 状态信息**\n"
            f"👤 用户：{user_info.get('user', 'Unknown')}\n"
            f"📦 订阅类型：{user_info.get('sku', 'Unknown')}\n"
            f"{advanced_limits_str}\n"
            f"💬 Chat权限：{'✅ 允许' if user_info.get('chat_enabled', False) else '❌ 拒绝'}"
        )
